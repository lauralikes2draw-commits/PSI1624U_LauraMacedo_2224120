...
namespace ProjetoFinal
{
    public partial class AvaliacoesCliente : Form
    {
        ...
        private void AtualizarTextoRating(Guna2RatingStar rating, Label label)
        {
            ...
            else label.Text = rating.Value.ToString("0") + "/5 selecionado";
        }
        ...
        private void EnviarAvaliacao_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                int n1 = Convert.ToInt32(Math.Round(guna2RatingStar1.Value));
                int n2 = Convert.ToInt32(Math.Round(guna2RatingStar2.Value));
                int n3 = Convert.ToInt32(Math.Round(guna2RatingStar3.Value));
                if (n1 <= 0 || n2 <= 0 || n3 <= 0)
                {
