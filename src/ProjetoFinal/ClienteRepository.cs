...
IF COL_LENGTH('dbo.Avaliacoes', 'NotaProfissional') IS NULL ALTER TABLE dbo.Avaliacoes ADD NotaProfissional INT NULL;
IF COL_LENGTH('dbo.Avaliacoes', 'NotaEspaco') IS NULL ALTER TABLE dbo.Avaliacoes ADD NotaEspaco INT NULL;
IF COL_LENGTH('dbo.Avaliacoes', 'NotaServico') IS NULL ALTER TABLE dbo.Avaliacoes ADD NotaServico INT NULL;
...
END;";
        ...
        public static ClienteMarcacaoInfo GetAvaliacaoPendente(int idCliente)
        {
            string sql = @"SELECT TOP 1 m.IdMarcacao, m.IdServico, m.IdProfissional, m.Servico, m.Profissional, m.DataMarcacao, m.Hora, m.Valor, m.Estado, m.DuracaoMinutos,
                ...
                LEFT JOIN dbo.Servicos s ON m.IdServico=s.IdServico
                LEFT JOIN dbo.Usuarios u ON m.IdProfissional=u.IdUsuario
                WHERE m.IdCliente=@IdCliente AND m.Estado IN ('Concluído','Concluido') AND ISNULL(m.Avaliada,0)=0 AND m.DataMarcacao<=CAST(GETDATE() AS DATE)
                ORDER BY m.DataMarcacao DESC, m.Hora DESC";
        ...
        public static void EnviarAvaliacao(int idCliente, int idMarcacao, int notaProfissional, int notaEspaco, int notaServico, string comentario)
        {
            ...
            int media = (int)Math.Round((notaProfissional + notaEspaco + notaServico) / 3.0);
            using (SqlConnection conn = Conexao.Conectar())
            {
                conn.Open();
                SqlTransaction tx = conn.BeginTransaction();
                try
                {
                    using (SqlCommand cmd = new SqlCommand(@"INSERT INTO dbo.Avaliacoes (IdCliente, IdProfissional, IdServico, IdMarcacao, Classificacao, NotaProfissional, NotaEspaco, NotaServico, Comentario, DataAvaliacao)
                                                            ...
                                                            UPDATE dbo.Marcacoes SET Avaliada=1 WHERE IdMarcacao=@IdMarcacao;", conn, tx))
                    {
                        cmd.Parameters.AddWithValue("@IdCliente", idCliente);
