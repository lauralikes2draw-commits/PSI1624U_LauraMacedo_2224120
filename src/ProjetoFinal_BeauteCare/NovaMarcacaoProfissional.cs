...
{
    public partial class NovaMarcacaoProfissional : Form
    {
        ...
        public NovaMarcacaoProfissional()
        {
            this.idProfissional = 0;
            dataEscolhida = DateTime.Today;
            InitializeComponent();
        ...
        private void RenderizarPassoCliente()
        {
            ...
            btnPropria.Tag = true;
            btnCliente.Click += EscolhaDestinoProfissional_Click;
            btnPropria.Click += EscolhaDestinoProfissional_Click;
        ...
        private Guna2Button CriarChipCategoria(string texto, bool ativo)
        {
            ...
            chip.Tag = texto;
            chip.Click += CategoriaProfissional_Click;
            return chip;
        }
        ...
        private Control CriarCardServico(ServicoInfo s)
        {
            ...
            add.Tag = s;
            add.Click += ServicoProfissional_Click;
        ...
        private void EscolhaDestinoProfissional_Click(object sender, EventArgs e)
        {
            Control controle = sender as Control;
            if (controle == null || !(controle.Tag is bool)) return;
            paraPropriaProfissional = (bool)controle.Tag;
            if (paraPropriaProfissional) clienteSelecionada = null;
            RenderizarPasso();
        }

        private void CategoriaProfissional_Click(object sender, EventArgs e)
        {
            Control controle = sender as Control;
        ...
        private void ServicoProfissional_Click(object sender, EventArgs e)
        {
            Guna2Button botao = sender as Guna2Button;
            ServicoInfo servico = botao == null ? null : botao.Tag as ServicoInfo;
            if (servico == null) return;

            if (!servicosSelecionados.Exists(x => x.IdServico == servico.IdServico))
                servicosSelecionados.Add(servico);

            if (lblResumoSelecionados != null)
                lblResumoSelecionados.Text = ResumoServicosSelecionados();

            botao.Text = "Adicionado ✓";
            botao.FillColor = Color.FromArgb(60, 170, 100);
        }
